...
Deno.serve(async (req) => {
	...
	try {
		...
		try {
			...
			await sendEmail(adminEmail, `General enquiry from ${name.trim()}`, adminHtml, undefined, {
				kind: 'enquiry_admin'
			});
		} catch (e) {
		...
		try {
			await sendEmail(
				email.trim(),
				'We received your enquiry — BME e-Serve',
				buildConfirmationEmail(name.trim(), message.trim(), logoUrl),
				undefined,
