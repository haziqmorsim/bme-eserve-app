...
import {
    ...
    recipientsByRole, notifyRecipients, reminderEmail
...
Deno.serve(async (req) => {
    ...
    try {
        ...
        for (const r of admins) {
            if (!r.email) continue;
            try {
                await sendEmail(r.email, `BME e-Serve App — ${overdue.length} enquiry(ies) awaiting reply`, html);
                emailed++;
            } catch (err) {
