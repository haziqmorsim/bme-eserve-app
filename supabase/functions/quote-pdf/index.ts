...
Deno.serve(async (req) => {
	...
	try {
		...
		if (partIds.length) {
			const { data: parts } = await admin.from('parts').select('id, price').in('id', partIds);
		...
		const centerText = (s: string, cx: number, yy: number, size = 10, f = font, color = INK) =>
			page.drawText(s ?? '', {
				x: cx - f.widthOfTextAtSize(s ?? '', size) / 2,
				y: yy,
				size,
				font: f,
				color
		...
		const terms = (await getSettings(admin, 'quote_terms'))?.trim();
		if (terms) {
			ensureSpace();
			text('Terms', M, y, 9, bold, MUTED);
			y -= 14;
			const maxW = W - 2 * M;
			let line = '';
			for (const w of terms.split(/\s+/)) {
				const test = line ? line + ' ' + w : w;
				if (line && font.widthOfTextAtSize(test, 8) > maxW) {
					text(line, M, y, 8, font, MUTED);
					y -= 11;
					ensureSpace();
					line = w;
				} else {
