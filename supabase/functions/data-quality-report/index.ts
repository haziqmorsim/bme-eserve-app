...
Deno.serve(async (req) => {
	...
	try {
		...
		const [boilersRes, assignRes, customersRes, componentsRes, partsRes, readingsRes, specsRes] =
			await Promise.all([
				...
				admin
					.from('parts')
					.select('id, component_id, part_number, name, price, price_min, price_max'),
		...
		add(
			...
			components
				...
				.map(
					(c: any) =>
						`${c.name} (${boilerById[c.boiler_id] ? boilerById[c.boiler_id].code : 'unknown boiler'})`
				)
		);
		...
		for (const r of admins) {
			...
			try {
				await sendEmail(
					r.email,
					`BME e-Serve App — weekly data check (${total} item(s))`,
					html,
					undefined,
					{ kind: 'data_quality' }
				);
		...
		return json(200, {
			ok: true,
			findings: total,
			breakdown: findings.map((f) => ({ check: f.title, count: f.items.length })),
			emailed
		});
	} catch (e) {
