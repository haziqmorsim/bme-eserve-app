...
import {
    THRESHOLD_HOURS, serviceClient, hoursAgoIso, hoursBetween, guard, 
    notifyRecipients, reminderEmail
    ...
    try {
        ...
        for (const c of overdue) {
            ...
            await notifyRecipients(
                admin, 
                [{ id: c.user_id, email: recipient?.email ?? null, full_name: recipient?.full_name ?? null }], 
                {
                    type: "cart_reminder",
                    quote_id: null, 
            ...
            try {
                await sendEmail(recipient.email, 'BME e-Serve App — items waiting in your quote list', html);
                emailed++;
            } catch (e) {
        ...
        await admin
            ...
            .in('user_id', overdue.map((c: any) => c.user_id));
