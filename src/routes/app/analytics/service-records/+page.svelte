<script lang="ts">
	...
	const filtered = $derived(
		data.records.filter((r: any) => {
			...
			if (!q) return true;
			return [r.project_label, r.boiler_label, r.part_label, r.part_name, r.notes].some((v: any) =>
	...
	const when = (d: string) =>
		new Date(d + 'T:00:00:00').toLocaleDateString('en-MY', {
			year: 'numeric',
			month: 'short',
			day: 'numeric'
		});
		...
		<div class="grid">
			...
			<label class="check wide">
				...
				<span>
					Premature failure
					<small
						>Exclude from service-life averages - it describes an incident, not a lifespan.</small
					>
				</span>
			</label>
		</div>
