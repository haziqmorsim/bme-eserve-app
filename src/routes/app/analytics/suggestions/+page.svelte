<script lang="ts">
	...
	const filtered = $derived(
		data.rows.filter((r: any) => {
			...
			if (!q) return true;
			return [r.query_text, r.suggested_part_number, r.actual_part_number, r.boiler_code].some(
...
<p class="intro">
	...
	override the customer's own feedbacl.
</p>
...
<div class="searchbar card">
	...
	<input
		type="search"
		placeholder="Search by message, parrt number, or boiler..."
		bind:value={search}
	/>
</div>
				...
				<div class="card row">
					...
					{:else}
						<div class="actions">
							<button
								class="act ok"
								disabled={busy.has(r.id) || !r.suggested_part_id}
								onclick={() => save(r, 'accepted', r.suggested_part_id)}
								>Suggestion was correct</button
							>
							<button class="act fix" disabled={busy.has(r.id)} onclick={() => (picking = r.id)}
								>Correct it...</button
							>
							<button
								class="act no"
								disabled={busy.has(r.id)}
								onclick={() => save(r, 'rejected', null)}>No part applies</button
							>
						</div>
					{/if}
				</div>
