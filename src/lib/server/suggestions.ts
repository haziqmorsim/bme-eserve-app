...
export type SuggestionInput = {
    /** Optional caller-supplied id. The chat route pre-generates one so it can
     *  return x-suggestion-id in the response headers, which must be sent
     *  before the stream (and therefore before the row can be written). */
    id?: string;
    userId: string | null;
    ...
    try {
        ...
        for (const s of pending) {
            ...
            const ok = await resolveSuggestion(
                ...
                { actualPartId: accepted ? s.suggested_part_id : null, source: 'quote' }
            );
            if (ok) labelled++;
        }
        return labelled;
