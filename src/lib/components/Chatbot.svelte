	...
	let messages = $state<Msg[]>([
		{
			role: 'assistant',
			content:
				'Hi! I can help you find your way around BME e-Serve App. You can also upload a photo of a boiler part and I will try to identify the right spare part. What are you looking for?'
		}
	]);
		...
		try {
			...
			if (res.status === 429) {
				...
				messages[idx].content =
					reason.trim() || "You have reached today's message limit. Please try again tomorrow.";
				return;
			}
			...
			messages[idx].content =
				'Sorry, something went wrong. Please try again or chat with us in WhatsApp.';
		} finally {
			loading = false;
	...
	function normalize(text: string): string {
		...
		while (i < lines.length) {
			if (isPipe(lines[i])) {
				...
				const rows =
					sepIdx >= 0 ? block.filter((_, idx) => idx !== sepIdx && idx !== sepIdx - 1) : block;
				for (const r of rows) {
					const cells = r
						.trim()
						.replace(/^\|/, '')
...
<div class="chat-root">
	{#if open}
		<div
			class="chat-panel"
			role="dialog"
			aria-label="BME e-Serve assistant"
			transition:fly={{ y: 16, duration: 220, easing: cubicInOut }}
		>
				...
				{#each messages as m, mi}
					<div class="chat-bubble {m.role}">
						...
						{#each segments(normalize(m.content)) as seg}
							{#if seg.type === 'link'}<a href={seg.href} onclick={() => (open = false)}
									>{seg.text}</a
								>
			...
			<div class="chat-input-row">
				<input
					type="text"
					placeholder="Ask me how to find something..."
					bind:value={input}
					onkeydown={onKey}
					disabled={loading}
				/>
				...
				<button
					...
					disabled={loading}
					aria-label="Upload a boiler part photo"
					title="Upload a boiler part photo"
				>
					<ImageIcon size={18} />
				</button>
				<button
					class="chat-send"
					onclick={send}
					disabled={loading || (!input.trim() && !pendingImage)}>Send</button
				>
			</div>
...
<style>
	...
	.chat-img-btn:hover:not(:disabled) {
		background-color: var(--bme-hover);
	}
