<script lang="ts">
	...
	function validate(): boolean {
		...
		if (!email) e.admin_email = 'Admin e-mail is required.';
			...
			e.admin_email = 'Enter a valid e-mail address.';
		...
		if (form.whats_new_enabled === 'on') {
			if (!(form.whats_new_version ?? '').trim())
				e.whats_new_version = "Required when What's New is on.";
			if (!(form.whats_new_content ?? '').trim())
				e.whats_new_content = "Required when What's New is on.";
		}

		if (!(form.privacy_policy ?? '').trim()) e.privacy_policy = 'Privacy policy cannot be empty.';
		if (!(form.terms_conditions ?? '').trim())
			e.terms_conditions = 'Terms and conditions cannot be empty.';
...
<div class="gen-wrap">
	<section class="card gen-card">
		<h2>Notifications</h2>
		<p class="gen-hint">
			Where staff notifications for new quotation requests and general enquiries are sent.
		</p>
		<div class="adm-form">
			<label
				>Admin E-mail <span class="required">*</span>
				<input
					type="email"
					class="w-50"
					bind:value={form.admin_email}
					placeholder="admin@boilermech.com"
					class:invalid={fieldErr.admin_email}
				/>
			...
			<label
				>Target Reply Time (days)
				<input
					class="w-25"
					bind:value={form.quote_reply_days}
					placeholder="3"
					class:invalid={fieldErr.quote_reply_days}
				/>
				{#if fieldErr.quote_reply_days}<span class="field-err">{fieldErr.quote_reply_days}</span
					>{/if}
			</label>
		</div>
	</section>
	...
	<section class="card gen-card">
		...
		<div class="adm-form">
			<label
				...
				<input
					class="w-25"
					bind:value={form.sla_warn_hours}
					placeholder="24"
					class:invalid={fieldErr.sla_warn_hours}
				/>
			...
			<label
				...
				<input
					class="w-25"
					bind:value={form.sla_overdue_hours}
					placeholder="48"
					class:invalid={fieldErr.sla_overdue_hours}
				/>
				{#if fieldErr.sla_overdue_hours}<span class="field-err">{fieldErr.sla_overdue_hours}</span
					>{/if}
			</label>
		</div>
	</section>
	...
	<section class="card gen-card">
		<h2>Quotations</h2>
		<p class="gen-hint">
			The prefix applies to newly created requests only, existing reference numbers are never
			rewritten.
		</p>
		<div class="adm-form">
			<label
				...
				<input
					class="w-50"
					bind:value={form.quote_ref_prefix}
					placeholder="BME"
					class:invalid={fieldErr.quote_ref_prefix}
				/>
				{#if fieldErr.quote_ref_prefix}<span class="field-err">{fieldErr.quote_ref_prefix}</span
					>{/if}
			</label>
			<label
				>Quote Validity (days)
				<input
					class="w-25"
					bind:value={form.quote_validity_days}
					placeholder="30"
					class:invalid={fieldErr.quote_validity_days}
				/>
				{#if fieldErr.quote_validity_days}<span class="field-err"
	...
	<section class="card gen-card">
		<h2>Operational</h2>
		<p class="gen-hint">
			Maintenance mode blocks customer access while staff keep working, so it can always be switched
			back off.
		</p>
		<div class="adm-form">
			<label
			...
			<label
				>Chatbot Daily Limit (per user)
				<input
					class="w-25"
					bind:value={form.chatbot_daily_limit}
					placeholder="50"
					class:invalid={fieldErr.chatbot_daily_limit}
				/>
	...
	<section class="card gen-card">
		...
		<div class="adm-form">
			...
			<div class="full toggle-field wn-field">
				...
				<label class="wn-version"
					>Version
					<input
						class="w-25"
						bind:value={form.whats_new_version}
						placeholder="1.0"
						class:invalid={fieldErr.whats_new_version}
					/>
					{#if fieldErr.whats_new_version}<span class="field-err">{fieldErr.whats_new_version}</span
						>{/if}
				</label>
				<label class="wn-version"
					>Contents
					<textarea
						...
						class:invalid={fieldErr.whats_new_content}
					></textarea>
				</label>
				{#if fieldErr.whats_new_content}<span class="field-err">{fieldErr.whats_new_content}</span
					>{/if}
			</div>
		</div>
	</section>
	...
	<section class="card gen-card">
		...
		<p class="gen-hint">
			Shown to all users on the Privacy Policy page. Blank lines separate paragraphs; a short line
			on its own becomes a heading.
		</p>
		<div class="adm-form">
			<label class="full">
				<textarea rows="14" bind:value={form.privacy_policy} class:invalid={fieldErr.privacy_policy}
				></textarea>
	...
	<section class="card gen-card">
		...
		<div class="adm-form">
			<label class="full">
				<textarea
					rows="14"
					bind:value={form.terms_conditions}
					class:invalid={fieldErr.terms_conditions}
				></textarea>
				{#if fieldErr.terms_conditions}<span class="field-err">{fieldErr.terms_conditions}</span
					>{/if}
			</label>
		</div>
	</section>
