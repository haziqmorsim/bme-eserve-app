<script lang="ts">
	...
	async function saveSpec() {
		...
		addToast(
			editSpec === 'new' ? 'Specification added successfully' : 'Specification updated successfully'
		);
		editSpec = null;
		await invalidateAll();
	}
	...
	async function saveReading() {
		...
		addToast(
			editReading === 'new' ? 'Readings added successfully' : 'Readings updated successfully'
		);
		editReading = null;
		await invalidateAll();
	}
...
<div class="bd-head">
	...
	<label class="bd-pick">
		...
		<select bind:value={boilerId}>
			{#each boilers as b (b.id)}<option value={b.id}>{b.code}{b.name ? ` - ${b.name}` : ''}</option
				>{/each}
		</select>
	</label>
</div>
...
{:else}
	<div class="adm-bar">
		...
		<button class="btn-primary" onclick={newSpec}>
	...
	<div class="adm-bar">
		...
		<div class="sec-right">
			<span class="sec-count"
				>{myReadings.length} of {sectionOptions.length} sections configured</span
			>
			<button class="btn-primary" onclick={newReading} disabled={freeSections.length === 0}
				>Add Readings</button
			>
		</div>
	</div>
					...
					{#each myReadings as r (r.id)}
						<tr>
							...
							<td style="text-align:center; vertical-align:middle;">
								<span class="state {(r.state ?? 'Normal').toLowerCase()}"
									>{r.state ?? 'Normal'}</span
								>
							</td>
...
{#if editSpec !== null}
	<Modal
		title={editSpec === 'new' ? 'Add Specification' : 'Edit Specification'}
		onclose={() => (editSpec = null)}
	>
		<div class="adm-form stack">
			<label
				>Label <span class="required">*</span>
				<input
					bind:value={specForm.label}
					placeholder="Thermal Efficiency"
					class:invalid={fieldErr.label}
				/>
...
{#if editReading !== null}
	<Modal
		title={editReading === 'new' ? 'Add Section Readings' : 'Edit Section Readings'}
		onclose={() => (editReading = null)}
	>
		<div class="adm-form stack">
			<label
				...
				<select bind:value={readingForm.section_key} class:invalid={fieldErr.section_key}>
					{#each sectionOptions as s (s.key)}
						<option
							value={s.key}
							disabled={usedKeys.has(s.key) && s.key !== readingForm.section_key}
						>
							{s.label}{usedKeys.has(s.key) && s.key !== readingForm.section_key
								? ' (already added)'
								: ''}
						</option>
					{/each}
			...
			<div class="mx">
				...
				{#each readingForm.metrics as m, i}
					<div class="mx-row">
						<input
							placeholder="Label (e.g. Flue Gas Temp)"
							bind:value={readingForm.metrics[i].label}
						/>
...
{#if deleteReading}
	<Modal title="Delete Section Readings" onclose={() => (deleteReading = null)}>
		<div class="modal-confirm">
			<p>
				Are you confirm to delete the readings for <strong
					>{sectionLabel(deleteReading.section_key)}</strong
				>?
			</p>
			{#if err}<p class="adm-err">{err}</p>{/if}
			<div class="modal-actions">
